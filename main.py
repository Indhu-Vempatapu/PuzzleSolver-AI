...
class PuzzleApp:
    ...
    def draw(self):
        self.screen.fill((240, 248, 255))  # AliceBlue
        ...
        # Move RESET to the left, SOLVE to the center
        # Place SOLVE & RESET centered between grids, under them
        # Horizontal button layout under both bottom grids
        # Center all 4 buttons under the two lower grids (Details & Auto Solve)
        button_y = HEIGHT // 2 + TILE_SIZE * 3 + 40  # just below the lower grids
        button_spacing = 20
        ...
        # Re-center search panel vertically
        ...
        # Show result text below navigation buttons
        # Position under algorithm panel
        # === STATUS TEXT UNDER ALGORITHM PANEL ===
# Set fixed spacing below the last radio button
        # === STATUS TEXT UNDER ALGORITHM PANEL (green box area) ===
        # === STATUS TEXT inside green box, below last radio button ===
        if self.panel.radio_buttons:
            last_button = self.panel.radio_buttons[-1]
            status_x = self.panel.rect.x + 10
            status_y = self.panel.rect.y + last_button.rect.bottom + 30  # ~30px below last radio button
